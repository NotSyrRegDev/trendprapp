...
const AddAdmin = ({navigation}) => {
  ...
  const { onAddAdmin, error, isLoading  } = useContext(AuthenticationContext);
  ...
  const handleAddAdmin = () => {
    onAddAdmin(fullName , phoneNumber , userEmail , password) ;
    setTimeout( () => {
    ...
    {isLoading ? (
      ...
      <TouchableOpacity
        className="text-white mt-10 rounded-lg text-sm px-6 py-4 mr-2 mb-2 "
          ...
          <Text style={styles.buttonText}>  انشاء حساب أدمن  </Text>
        </TouchableOpacity>
    )}
